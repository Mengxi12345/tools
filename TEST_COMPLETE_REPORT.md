...
- **GET /api/v1/health**
  - 状态：✅ 成功
  - 响应：返回健康状态信息
  - 验证：status: "UP", service: "content-aggregator"
...
**现象**：GET /api/v1/platforms 和 POST /api/v1/platforms 返回 500
**可能原因**：
1. 数据库表未创建（Hibernate ddl-auto 可能未正确执行）
2. Repository 查询异常
3. 实体类映射问题

**解决方案**：
- 检查 Hibernate 配置
- 确保 ddl-auto 设置为 update 或 create
- 检查应用日志中的具体错误信息
...
6. ✅ **API 文档**：Swagger/OpenAPI 文档正常生成
